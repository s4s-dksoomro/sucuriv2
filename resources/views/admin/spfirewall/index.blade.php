...
<div class="panel panel-default panel-main">
      <div class="panel-body  row">
          <div class="col-lg-8">
          <div  class="setting-title" ><h3>WAF Status
  ...
  <p class="text-info">This setting was last changed 2 days ago</p>
          ...
          <div class="col-lg-4 right ">
           ...
           <select  settingid="{{$zoneSetting->where('name','waf')->first()->id }}"  style="width: 200px;" class="select2 changeableSetting" id="waf" name="waf">
                <option {{ $waf == "disabled" ? "selected":"" }} value="disabled">Disabled</option>
                 <option {{ $waf == "enabled" ? "selected":"" }} value="enabled">Enabled</option>
            </select>
            @elseif($zoneSetting->where('name','zoneshield')->first()->value=="0")
            ...
            @else
              ...
              <div class="alert alert-info">WAF can only be enabled when OriginShield is turned off.
</div>
                                            ...
                                            data-domain="{{ $event->domain }}" 
                                             data-clientip="{{ $event->client_ip }}" 
                                              data-country="{{ $event->country }}" 
                                               data-useragent="@if($event->user_agent!="") {{ json_decode($event->user_agent)->client }} @endif" 
   ...
   <div class="modal-dialog modal-lg" >
    <div class="modal-content">
      ...
      <div class="col-lg-12 text-right pull-right">
            <input class="btn btn-success createRuleFromEvent" name="" value="Create Firewall Rule Based on this Event" type="submit">
        </div>
        </div>
</div></div>
...
<div class="panel panel-white">
            <div class="tabbable full-width-tabs">
                ...
                @if($zoneSetting->where('name','waf')->first()->value=="enabled")
              <ul class="nav nav-tabs">
                <li class="active"><a href="#wafPanel" data-toggle="tab">Firewall Policies</a></li>
                <li><a href="#ipfirewall" data-toggle="tab">WAF Rules</a></li>
            ...
            <div class="panel-body">
              <div class="tab-content">
                <div class="tab-pane active" id="wafPanel">
    ...
    <div class="panel panel-default panel-main">
        ...
        <div class="pull-right">
          ...
          <a id="addWafRuleBtn" class="btn btn-primary editWafRule" data-toggle="modal" data-target="#rule-add-modal">Add Rule</a>
    </div>
  </div>
      ...
      <input type="hidden" name="csrftoken" value="{{csrf_token()}}" >
        ...
        <table style="display: none;" class="table table-bordered table-striped table-condensed">
            ...
            <table class="table table-bordered table-striped table-condensed">
                        ...
                        @foreach ($rules as $rule)
                            <tr id="rule_{{ $rule->id }}" data-entry-id="{{ $rule->id }}">
                                ...
                                <td>
                                    ...
                                    Complex
                                    @elseif($rule->SpCondition->count()>0)
                              ...
                              {{ $rule->action }}
                                ...
                                <td>
                                  ...
                                  <a style="margin:20px;"  data-toggle="modal" data-target="#rule-edit-modal_{{ $rule->id }}" class="editWafRule" rule-id="{{$rule->id}}" class="btn btn-secondary">
                                    <i class="glyphicon glyphicon-edit"></i>
                                    </a>

                                    <a class="deleteSpWAFRule" rule-id="{{$rule->id}}" class="btn btn-default">
                ...
                <div class="tab-pane" id="ipfirewall">
...
@foreach($wafPackages as $wafPackage)
 ...
 <div class="panel panel-default panel-main">
      <div class="panel-body  ">
      <div class="row accordian-heading collapsed" data-toggle="collapse" data-target="#collapse_{{ $wafPackage->id }}">
          <div class="col-lg-12">
  ...
  <p class="text-info">This setting was last changed 2 days ago</p>
      ...
      <div class="collapse wafGroups" id="collapse_{{ $wafPackage->id }}">
...
<div class="modal" id="rule-add-modal" data-reveal>
...
<form method="post" action="addWAFRule" class="WAFRuleForm">
  <input type="hidden" name="csrftoken" value="{{csrf_token()}}" >
<p><strong>Rule Name:</strong> Make it easy to remember </p>
<input class="form-control" required="" placeholder="Rule Name" id="wafrulename" name="name"  type="text">

<p style="padding-top: 20px;"><strong>If:</strong></p>
<div id="settings" class="settings">
<div  style="padding-bottom: 15px;"  class="row">
<div class="col-lg-5">
  <select  oldValue="Ip"  name="scope[]" tabindex="-1" title="" class="select2 scope">
...
<div class="col-lg-3 valueDiv">
  <select id="scopeVal"  oldValue=""  name="data[]" class="select2 value form-control">
    <option value="on">YES</option>
    <option value="of">NO</option>
  </select>
</div>
<div class="col-lg-3 addCont">
   <a href="#" class="btn btn-link copy-condition">
  <i class="glyphicon glyphicon-duplicate"></i>

</a>

 <a style="display: none;" href="#" class="btn btn-link remove-condition removeAction">
  <i class="glyphicon glyphicon-remove"></i>

</a>
</div>
<div style="padding-top: 20px; display: none;" class="col-lg-12 extraDiv">
  <label>Exact Match <input type="checbox" name="extra[]" class="form-control extra"></label>
  </div>
</div>
 </div>
...
<div class="row">
  <div class="col-lg-4">Then the action is: </div>
  <div class="col-lg-8">
   <select id="rule_action" name="action" class="no-skin" required>
                                                    <option value="Allow">Allow</option>
                                                    <option value="Block">Block</option>
...
<div class="modal rule-edit-modal" id="rule-edit-modal_{{ $rule->id }}" >

   <div class="modal-dialog modal-lg" >
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Edit WAF Rule</h4>
      </div>
      <div class="modal-body">
...
<form method="post" action="editWafRule" class="wafRuleEditForm">
  <input type="hidden" name="zid" value="{{ $zone->id }}">
<input type="hidden" name="ruleid" value="{{ $rule->id }}">

  <input type="hidden" name="csrftoken" value="{{csrf_token()}}" >
<p><strong>Rule Name:</strong> Make it easy to remember </p>
<input class="form-control"  value="{{ $rule->name }}"  required="" placeholder="Rule Name" name="name"  type="text">

<p style="padding-top: 20px;"><strong>If:</strong></p>
<div id="settings" class="settings">

    @foreach ($rule->SpCondition as $condition)

<div  style="padding-bottom: 15px;"  class="row">
<div class="col-lg-5">
  <select  oldValue="{{ $condition->scope }}"   name="scope[]" tabindex="-1" title="" class="select2 scope">
...
<div class="col-lg-3 valueDiv">
  <select  oldValue="{{ $condition->data }}"    name="data[]" class="select2 value form-control">
    <option value="on">YES</option>
    <option value="of">NO</option>
  </select>
<input placeholder="" type="hidden"  name="data2[]" class=" value form-control" >
...
<div class="col-lg-3 addCont">
  ...
  @if($rule->SpCondition->first()->id != $condition->id)
...
<input class="actionid" type="hidden" name="actionID[]" value="{{ $condition->id }}">
<input class="deleteinput" type="hidden" name="delete[]" value="0">

</div>
<div style="padding-top: 20px; display: none;" class="col-lg-12 extraDiv">
  <label>Exact Match <input type="checkbox" name="extra[]" class="form-control extra"></label>
  </div>
</div>
@endforeach
 </div>
